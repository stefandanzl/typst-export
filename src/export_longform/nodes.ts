		...
		if (address_is_image_file(this.content)) {
			...
			if (file === undefined) {
				...
				if (this.custom_label) {
					...
					// Register label for duplicate detection
					register_label(data, p.label, "Plot/Image");
				} else {
					p.label = undefined;
				}
...
export class Table implements node {
	...
	static get_regexp(): RegExp {
		// Match Markdown tables + next non-table line (caption/label)
		return /(^\|.*\|\r?\n^\|[ \t]*[:-]+.*\|\r?\n(?:^\|.*\|\r?\n)*)(^(?!\|).*)/gm;
	}
	...
	static build_from_match(
		...
		const fullTable = match[1].trim(); // full table markdown text
		const afterLine = (match[2] || "").trim(); // possible caption/label line

		// Split the table into lines
		const lines = fullTable.split(/\r?\n/);
		if (lines.length < 2) throw new Error("Invalid markdown table format.");

		const headerLine = lines[0];
		const dataLines = lines.slice(2); // skip header + separator line

		// Parse headers
		const headers = Table.parseTableRow(headerLine);

		// Parse data rows
		const rows = dataLines
			.filter((l) => l.trim().startsWith("|"))
			.map((line) => Table.parseTableRow(line));

		// Extract caption and optional <@label>
		let caption: string | undefined;
		let tableId: string | undefined;

		if (afterLine) {
			const labelMatch = /<@([^>]+)>/.exec(afterLine);
			if (labelMatch) {
				tableId = labelMatch[1];
				caption = afterLine.replace(/<@[^>]+>/, "").trim() || undefined;
			} else {
				caption = afterLine.trim() || undefined;
			}
		}

		return new Table(headers, rows, caption, tableId);
	}
	...
	// Reusable row parser
	static parseTableRow(line: string): string[] {
	...
	async unroll(data: metadata_for_unroll): Promise<node[]> {
		...
		// Register label if it exists
		if (this.label) {
			register_label(data, this.label, "Table");
		}
...
export class MarkdownImage implements node {
	...
	async unroll(
		...
		if (this.custom_label) {
			...
			// Register label for duplicate detection
			register_label(data, this.label, "MarkdownImage");
		} else {
...
export class UnrolledWikilink implements node {
	...
	constructor(
		...
		this.unroll_data = {
			...
			labels: unroll_data.labels, // Share the same label set
		};
		this.address = address;
		this.attribute = attribute;
		this.address = address;
